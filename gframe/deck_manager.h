...
public:
	...
	static uint32_t LoadDeckFromBuffer(Deck& deck, uint32_t* dbuf, uint32_t mainc, uint32_t powersc, uint32_t sidec, RITUAL_LOCATION rituals_in_extra = RITUAL_LOCATION::DEFAULT);
	static uint32_t LoadDeck(Deck& deck, const cardlist_type& mainlist, const cardlist_type& sidelist, const cardlist_type* extralist = nullptr, RITUAL_LOCATION rituals_in_extra = RITUAL_LOCATION::DEFAULT, const cardlist_type* powerslist = nullptr);
	static bool LoadSide(Deck& deck, uint32_t* dbuf, uint32_t mainc, uint32_t powersc, uint32_t sidec, bool rituals_in_extra);
